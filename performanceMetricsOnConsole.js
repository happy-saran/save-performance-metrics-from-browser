...
getFileType = (fileExtension, initiatorType) => {
	if (fileExtension) {
...
(() => {
	...
	console.info('Performance metrics data collected');
	
	// call API to store data to back end
	postDataToAPI(data);
})();


async function postDataToAPI(data) {
	console.info('Calling API to store data to db');
	console.log(data);
	const url = 'URL/TO/STORE/DATA';
	try {
		...
		response.json()
			.then(data => {
				console.log('Performance metrics saved to backend');
			});
	}
	catch (err) {
		console.log(err);
